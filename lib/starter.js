 ...
 *****************************************************************/
 ...
 const createProject = async (projectName) => {
   ...
   try {
     ...
     console.log("[ 1 / 4 ] 🔍  copying project...");
     console.log("[ 2 / 4 ] 🚚  fetching node_modules...");
 
     await copyProjectFiles(projectName, template);
     await updatePackageJson(projectName);
     console.log("[ 3 / 4 ] 🔗  linking node_modules...");

     await copyEnv(projectName,template);
 
     console.log("[ 4 / 4 ] 🔗  copying .env...");
     console.log(
       "\u001b[2m──────────────────────────────────────────\u001b[22m"
     );
  ...
  * @description Duplicate the template.
  */
   const copyEnv = async (destination, directory) => {
    return new Promise((resolve, reject) => {
      const source = path.join(__dirname, `./${directory}/.env.example`);
      const options = {
        clobber: true,
        stopOnErr: true,
      };
      ...
      ncp.limit = 16;
      const nDest = `${destination}/.env`
      ncp(source, nDest, options, function (err) {
        if (err) reject(err);
        resolve();
      });
    });
