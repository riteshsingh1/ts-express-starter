 ...
 * @returns Respons
 */
const RetryOTP = async (request: Request, response: Response) => {
    ...
    if (otpmodel.emailOrMobile === 'mobile') {
      ...
      if (newOtp) {
        if (otpmodel.purpose === 'EXISTING_ACCOUNT_VIA_MOBILE') {
          ...
          await sendOtp(parseInt(otpmodel.userdata), REGISTER_USER_SMS_TEMPLATE.replace('$$$$', newOtp?.otp.toString()))
        }
      }
      return successResponse(request, response, ApplicationCodes.OK, replyToken)
    }
...
const getStateAndCityBasedOnPincode = async (request: Request, response: Response) => {
